...
{
    char d = p->content[p->current_pos];
...
{
    if (readrange(p, '0', '9') && !readeof(p))
    {

        while (readrange(p, '0', '9') && !readeof(p))
        {
            continue;
        }
...
int readid(struct parser *p)
{
    ...
    {
        while (readrange(p, 'a', 'z') || readrange(p, 'A', 'Z') || readint(p) || readchar(p, '_'))
        {
            continue;
        }
...
{
    return readint(p) && readchar(p, '.') && (readint(p) || 1);
}
static int readfloat_frac(struct parser *p)
{
    return readchar(p, '.') && readint(p);
}
static int readfloat_exp(struct parser *p)
{
    return (readchar(p, 'e') || readchar(p, 'E')) && (readchar(p, '-') || readchar(p, '+') || 1) && readint(p);
}
int readfloat(struct parser *p)
{
    return (readchar(p, '-') || readchar(p, '+') || 1) && (readfloat_dec(p) || readfloat_frac(p)) && (readfloat_exp(p) || 1);
}
